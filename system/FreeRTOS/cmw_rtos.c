...
#define CMW_THREAD_PRIORITY           ((unsigned long)configMAX_PRIORITIES-1)
...
cmw_result_t host_rtos_create_thread( /*@out@*/ host_thread_type_t* thread, void(*entry_function)( uint32_t ), const char* name, /*@null@*/ void* stack, uint32_t stack_size, uint32_t priority )
{
    return host_rtos_create_thread_with_arg( thread, entry_function, name, stack, stack_size, priority, 0 );
}

cmw_result_t host_rtos_create_thread_with_arg( /*@out@*/ host_thread_type_t* thread, void(*entry_function)( uint32_t ), const char* name, /*@null@*/ void* stack, uint32_t stack_size, uint32_t priority, uint32_t arg )
{
    if ( stack )
    {
        ...
        *thread = xTaskCreateStatic( (TaskFunction_t)entry_function, name, (unsigned short)(adjusted_stack_size / sizeof( portSTACK_TYPE )), (void*)arg, (unsigned portBASE_TYPE) priority, stack_buffer, tcb_buffer );
        ...
        return ( *thread == NULL ) ? CMW_THREAD_CREATE_FAILED : CMW_SUCCESS;
    } else {
        ...
        return ( result == (signed portBASE_TYPE) pdPASS ) ? CMW_SUCCESS : CMW_THREAD_CREATE_FAILED;
    }
}

cmw_result_t host_rtos_create_configed_thread(  /*@out@*/ host_thread_type_t* thread, void(*entry_function)( uint32_t ), const char* name, /*@null@*/ void* stack, uint32_t stack_size, uint32_t priority, host_rtos_thread_config_type_t *config )
{
 ...
 *
 * @returns CMW_SUCCESS on success, HEXIN_ERROR otherwise
 */
cmw_result_t host_rtos_finish_thread( host_thread_type_t* thread )
{
 ...
 * @returns CMW_SUCCESS on success, HEXIN_ERROR otherwise
 */
cmw_result_t host_rtos_join_thread( host_thread_type_t* thread )
{
 ...
 * @returns CMW_SUCCESS on success, HEXIN_ERROR otherwise
 */
cmw_result_t host_rtos_delete_terminated_thread( host_thread_type_t* thread )
{
 ...
 * @returns CMW_SUCCESS on success, HEXIN_ERROR otherwise
 */
cmw_result_t host_rtos_init_semaphore(  /*@special@*/ /*@out@*/ host_semaphore_type_t* semaphore ) /*@allocates *semaphore@*/  /*@defines **semaphore@*/
{
    ...
    *semaphore = xSemaphoreCreateCounting( (unsigned portBASE_TYPE) 0x7fffffff, (unsigned portBASE_TYPE) 0 );
    ...
    return ( *semaphore != NULL ) ? CMW_SUCCESS : CMW_SEMAPHORE_ERROR;
    /*@+compdef@*/
}
...
cmw_result_t host_rtos_get_semaphore( host_semaphore_type_t* semaphore, uint32_t timeout_ms, /*@unused@*/ hexin_bool_t will_set_in_isr ) /*@modifies internalState@*/
{
 ...
 *                        : HEXIN_ERROR if an error occurred
 *
 */

cmw_result_t host_rtos_set_semaphore( host_semaphore_type_t* semaphore, hexin_bool_t called_from_ISR )
{
    signed portBASE_TYPE result;

    if ( called_from_ISR == HEXIN_TRUE )
    {
        signed portBASE_TYPE xHigherPriorityTaskWoken;
        result = xSemaphoreGiveFromISR( *semaphore, &xHigherPriorityTaskWoken );

        hexin_assert( "Unable to set semaphore", result == pdTRUE );
    ...
    return ( result == pdPASS )? CMW_SUCCESS : CMW_SEMAPHORE_ERROR;
}
 ...
 *
 */
cmw_result_t host_rtos_deinit_semaphore( /*@special@*/ host_semaphore_type_t* semaphore ) /*@releases *semaphore@*/
{
    if (semaphore != NULL)
    {
 ...
 */
cmw_time_t host_rtos_get_time( void )  /*@modifies internalState@*/
{
    return (cmw_time_t) ( xTaskGetTickCount( ) * ( 1000 / configTICK_RATE_HZ ) );
}
 ...
 *
 */
cmw_result_t host_rtos_delay_milliseconds( uint32_t num_ms )
{
    uint32_t remainder;
...
void vApplicationStackOverflowHook( TaskHandle_t *pxTask, signed portCHAR *pcTaskName )
{
    ...
    hexin_assert("Stack Overflow Detected", 0 != 0);
}

void vApplicationMallocFailedHook( void )
{
    WPRINT_RTOS_DEBUG(("Heap is out of memory during malloc\n"));
    hexin_assert("Malloc failed!", 0 != 0);
}


cmw_result_t host_rtos_init_queue( /*@special@*/ /*@out@*/ host_queue_type_t* queue, void* buffer, uint32_t buffer_size, uint32_t message_size ) /*@allocates *queue@*/  /*@defines **queue@*/
{
...
cmw_result_t host_rtos_push_to_queue( host_queue_type_t* queue, void* message, uint32_t timeout_ms )
{
...
cmw_result_t host_rtos_pop_from_queue( host_queue_type_t* queue, void* message, uint32_t timeout_ms )
{
    signed portBASE_TYPE retval;
...
cmw_result_t host_rtos_deinit_queue( /*@special@*/host_queue_type_t* queue ) /*@releases *queue@*/
{
    vQueueDelete( *queue );
    return CMW_SUCCESS;
}
