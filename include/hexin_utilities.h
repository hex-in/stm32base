...
/**
 * Macro to verify that result value of function x is HEXIN_SUCCESS
 * if not, return with result value;
 */
#define HEXIN_VERIFY(x)                               {hexin_result_t res = (hexin_result_t)(x); if (res != HEXIN_SUCCESS){return res;}}

/**
 * Verify the expression, if the result is not HEXIN_SUCCESS, jump to the label
 */
#define HEXIN_VERIFY_GOTO( expr, res_var, label )     {res_var = (expr); if (res_var != HEXIN_SUCCESS){goto label;}}
 ...
 */
#define hexin_jump_when_not_true( condition, label ) \
    do \
    { \
        if( ( condition ) == 0 ) \
        { \
 ...
 */
#define hexin_action_jump_when_not_true( condition, jump_label, action ) \
    do \
    { \
        if( ( condition ) == 0 ) \
        { \
...
#define hexin_htol16_ua_store( val, bytes ) \
    do \
    { \
        uint16_t _val = (val); \
 ...
 */
#define hexin_hex_nibble_to_ascii_char( cur_char ) (uint8_t)( (cur_char) < 10  ? ( (uint8_t)'0' +  (cur_char) ) : ( (uint8_t)'a' + ( (cur_char) - 10 ) ) )

/**
 * Macro to perform XOR Swap
 */
#define HEXIN_XOR_SWAP( val1, val2 )               do { val1 = val1 ^ val2; val2 = val1 ^ val2; val1 = val2 ^ val1; } while ( 0 )
 ...
 */
#ifdef HEXIN_ENABLE_MALLOC_DEBUG
...
#endif /* ifdef HEXIN_ENABLE_MALLOC_DEBUG */

/**
 *  Define macros to assist operation on host MCUs that require aligned memory access
 */
#ifndef HEXIN_HOST_REQUIRES_ALIGNED_MEMORY_ACCESS

#define HEXIN_MEMCPY(destination, source, size)   memcpy(destination, source, size)

#define HEXIN_WRITE_16( pointer, value )      (*((uint16_t*)pointer) = value)
#define HEXIN_WRITE_32( pointer, value )      (*((uint32_t*)pointer) = value)
#define HEXIN_READ_16( pointer )              *((uint16_t*)pointer)
#define HEXIN_READ_32( pointer )              *((uint32_t*)pointer)

#else /* HEXIN_HOST_REQUIRES_ALIGNED_MEMORY_ACCESS */

#define HEXIN_MEMCPY( destination, source, size )   mem_byte_cpy( destination, source, size )

#define HEXIN_WRITE_16( pointer, value )      do { ((uint8_t*)pointer)[0] = (uint8_t)value; ((uint8_t*)pointer)[1]=(uint8_t)(value>>8); } while(0)
#define HEXIN_WRITE_32( pointer, value )      do { ((uint8_t*)pointer)[0] = (uint8_t)value; ((uint8_t*)pointer)[1]=(uint8_t)(value>>8); ((uint8_t*)pointer)[2]=(uint8_t)(value>>16); ((uint8_t*)pointer)[3]=(uint8_t)(value>>24); } while(0)
#define HEXIN_READ_16( pointer )              (((uint8_t*)pointer)[0] + (((uint8_t*)pointer)[1] << 8))
#define HEXIN_READ_32( pointer )              (((uint8_t*)pointer)[0] + ((((uint8_t*)pointer)[1] << 8)) + (((uint8_t*)pointer)[2] << 16) + (((uint8_t*)pointer)[3] << 24))

#endif /* HEXIN_HOST_REQUIRES_ALIGNED_MEMORY_ACCESS */
 ...
 */
#define HEXIN_NEVER_TIMEOUT                         (0xFFFFFFFF)
#define HEXIN_WAIT_FOREVER                          (0xFFFFFFFF)
#define HEXIN_NO_WAIT                               (0)
 ...
 */
#define HEXIN_ETHER_ADDR_STR_LEN                    (18)
#define HEXIN_ETHER_ADDR_LEN                        ( 6)
...
/**
 ******************************************************************************
 * Convert WEP security key to the format used by HEXIN
 *
 ...
 */
char* hexin_ether_ntoa( const uint8_t *ea, char *buf, uint8_t buf_len );
 ...
 */
uint32_t hexin_ascii_to_hex( const char *ascii_buffer, uint8_t *buffer, uint32_t max_buffer_length );
